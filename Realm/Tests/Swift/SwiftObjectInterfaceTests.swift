...
class SwiftOptionalNumberObject: RLMObject {
    ...
    dynamic var floatCol: NSNumber? = NSNumber(value: 2.2 as Float)
    dynamic var doubleCol: NSNumber? = NSNumber(value: 3.3 as Double)
...
class SwiftObjectInterfaceTests: RLMTestCase {
    ...
    func testOptionalNSNumberProperties() {
        ...
        XCTAssertEqual(NSNumber(value: 2.2 as Float), no.floatCol!)
        ...
        try! realm.transaction {
            no.intCol = 1.1
            no.floatCol = NSNumber(value: 2.2 as Float)
            no.doubleCol = 3.3
            no.boolCol = false
        }
